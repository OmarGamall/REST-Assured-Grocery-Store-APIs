...
- **Centralized HTTP Client Wrapper (RestHelper)**: Implements a custom fluent HTTP builder client wrapping REST Assured. This decouples the API models from REST Assured, centralizes request logging, and eliminates repeating execution boilerplate across all endpoints.
- **Global Timeouts and SSL Bypass**: Configures global connection and socket (read) timeouts using `RestAssuredConfig` and `HttpClientConfig` inside [ApiConfigProvider](file:///d:/Edu/Omar%20Courses-Referances/APIs/RestAssured/GroceryStoreAPIs/src/main/java/com/grocerystore/apis/ApiConfigProvider.java) to protect the suite from hanging indefinitely. It also configures relaxed SSL verification globally via `setRelaxedHTTPSValidation()` to support seamless testing on local or internal staging environments that use self-signed certificates.
...
- **Thread-Safe Request Specifications**: Avoids global static configurations (such as `RestAssured.requestSpecification`) to prevent state leakage and thread collisions. Instead, isolated request specifications are dynamically generated per request using a factory pattern inside the [ApiConfigProvider](file:///d:/Edu/Omar%20Courses-Referances/APIs/RestAssured/GroceryStoreAPIs/src/main/java/com/grocerystore/apis/ApiConfigProvider.java) class.
...
- **`api.connection.timeout`** (in [env.properties](file:///d:/Edu/Omar%20Courses-Referances/APIs/RestAssured/GroceryStoreAPIs/src/test/resources/env.properties)): The HTTP connection establishment timeout in milliseconds. Defaults to `5000` (5 seconds).
- **`api.socket.timeout`** (in [env.properties](file:///d:/Edu/Omar%20Courses-Referances/APIs/RestAssured/GroceryStoreAPIs/src/test/resources/env.properties)): The HTTP socket/read inactive timeout in milliseconds. Defaults to `10000` (10 seconds).
